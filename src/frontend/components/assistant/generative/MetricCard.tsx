 ...
 *
 * Registers (via `makeAssistantToolUI`) a KPI card for tool-call parts whose
 * `toolName === "showMetric"`. While the tool runs we show a skeleton; once the
 * structured `result` arrives we render the value, an optional delta pill, and a
 * supporting hint.
 *
 * Mounting the returned component anywhere inside the `AssistantRuntimeProvider`
 * registers the renderer (see `generative/index.tsx`).
 */
...
/** Tool-UI component for `showMetric`. Render inside the runtime provider. */
